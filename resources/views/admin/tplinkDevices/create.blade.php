...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.tplinkDevice.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.tplink-devices.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group {{ $errors->has('online') ? 'has-error' : '' }}">
                ...
                @foreach(App\TplinkDevice::ONLINE_RADIO as $key => $label)
                    <div>
                        <input id="online_{{ $key }}" name="online" type="radio" value="{{ $key }}" {{ old('online', 0) === (string)$key ? 'checked' : '' }}>
